...
if __name__ == "__main__":
    """Запуск блока 10 для тестирования"""
    import pandas as pd
    import logging
    import sys
    import os
    ...
    try:
        ...
        print(f"✅ Загружены данные для извлечения признаков:")
        print(f"   - Book: {len(book_df)} строк")
        ...
        if not book_df.empty:
            ...
            print(f"   - Биржи в book: {exchanges}")
        ...
        # Конфигурация для извлечения признаков
        config = {
            'time': {
                'tick_size_ms': 10
            },
        ...
        # Запускаем блок 10
        print("🚀 Запускаем блок 10: Извлечение признаков для детекторов...")
        start_time = time.time()
        ...
        print(f"✅ Блок 10 выполнен за {execution_time:.2f} секунд!")
        print(f"📊 Результат извлечения признаков:")
        
        # Анализируем результаты
        if not features_df.empty:
            print(f"   - Признаки: {len(features_df)} записей")
            print(f"   - Биржи в признаках: {features_df['exchange'].unique() if 'exchange' in features_df.columns else 'N/A'}")
            
            if 'primary_detector' in features_df.columns:
                detectors = features_df['primary_detector'].value_counts()
                print(f"   - Основные детекторы: {dict(detectors)}")
            
            if 'overall_detector_score' in features_df.columns:
                avg_score = features_df['overall_detector_score'].mean()
                print(f"   - Средний общий скоринг: {avg_score:.4f}")
        
        print("✅ Блок 10 успешно протестирован!")
