...
if __name__ == "__main__":
    """Запуск блока 07 для тестирования"""
    import pandas as pd
    import logging
    import sys
    import os
    ...
    print("🧪 ТЕСТ БЛОКА 07: Trade Tape")
    print("=" * 50)
    ...
    try:
        ...
        # Конфигурация для анализа ленты сделок
        config = {
            'windows': {
        ...
        # Запускаем блок 07
        print("🚀 Запускаем блок 07: Анализ ленты сделок...")
        start_time = time.time()
        ...
        print(f"✅ Блок 07 выполнен за {execution_time:.2f} секунд!")
        print(f"📊 Результат анализа:")
        print(f"   - Проанализировано trades: {len(tape_df)} строк")
        
        # Проверяем результат
        if not tape_df.empty:
            print(f"   - Биржи в ленте: {tape_df['exchange'].unique()}")
            print(f"   - Стороны: {tape_df['aggressor'].unique()}")
            ...
            for exchange in tape_df['exchange'].unique():
                ...
                if 'aggression_type' in ex_data.columns:
                    ...
                    print(f"     Агрессия: {aggression_types.to_dict()}")
        
        print("✅ Блок 07 успешно протестирован!")
