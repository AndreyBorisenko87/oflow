...
# Для обратной совместимости с pipeline
def sync_time(
    trades_df: pd.DataFrame,
    depth_df: pd.DataFrame,
    config: Dict
    ...
    """
    Алиас для функции run_time_sync
    """
    return run_time_sync(trades_df, depth_df, config)


if __name__ == "__main__":
    """Запуск блока 04 для тестирования"""
    import pandas as pd
    import logging
    import sys
    import os
    ...
    print("🧪 ТЕСТ БЛОКА 04: SyncTime")
    print("=" * 50)
    
    try:
        # Загружаем реальные данные от блока 03 (нормализованные)
        trades_df = pd.read_parquet("../../../data/normalized/trades.parquet")
        depth_df = pd.read_parquet("../../../data/normalized/book_top.parquet")
        
        print(f"✅ Загружены нормализованные данные:")
        ...
        # Запускаем блок 04
        print("🚀 Запускаем блок 04: Синхронизация времени...")
        import time
        ...
        print(f"✅ Блок 04 выполнен за {execution_time:.2f} секунд!")
        print(f"📊 Результат синхронизации:")
        print(f"   - Синхронизированные trades: {len(synced_trades)} строк")
        print(f"   - Синхронизированные depth: {len(synced_depth)} строк")
        
        # Проверяем изменения
        if len(synced_trades) != len(trades_df):
            change = len(synced_trades) - len(trades_df)
            print(f"   - Изменение trades: {change:+d} строк")
        
        if len(synced_depth) != len(depth_df):
            change = len(synced_depth) - len(depth_df)
            print(f"   - Изменение depth: {change:+d} строк")
        
        print("✅ Блок 04 успешно протестирован!")
