...
def restore_quotes(
    ...
    total_exchanges = len(depth_df['exchange'].unique())
    logger.info(f"Обработка {total_exchanges} бирж...")
    
    for i, exchange in enumerate(depth_df['exchange'].unique(), 1):
        logger.info(f"Биржа {i}/{total_exchanges}: {exchange}")
        ex_data = depth_df[depth_df['exchange'] == exchange]
        
        total_buckets = len(ex_data['time_bucket'].unique())
        logger.info(f"  Временных окон: {total_buckets}")
        
        # Группируем по временным окнам
        for j, bucket_time in enumerate(ex_data['time_bucket'].unique(), 1):
            if j % 1000 == 0:  # Прогресс каждые 1000 окон
                logger.info(f"    Обработано {j}/{total_buckets} окон ({j/total_buckets*100:.1f}%)")
...
def calculate_spreads(quotes_df: pd.DataFrame, config: Dict = None) -> pd.DataFrame:
    """
...
if __name__ == "__main__":
    """Запуск блока 05 для тестирования"""
    import pandas as pd
    import logging
    import sys
    import os
    ...
    print("🧪 ТЕСТ БЛОКА 05: Best Prices")
    print("=" * 50)
    
    try:
        # Загружаем уменьшенные тестовые данные для быстрого тестирования
        depth_df = pd.read_parquet("../../../data/test_small/book_top_small.parquet")
        
        print(f"✅ Загружены синхронизированные данные:")
        ...
        # Конфигурация для восстановления цен
        config = {
            'best_prices': {
                'tick_size_ms': 10,
                'log_progress': True,
        ...
        # Запускаем блок 05
        print("🚀 Запускаем блок 05: Восстановление лучших цен...")
        start_time = time.time()
        
        quotes_df = run_best_prices(depth_df, config)
        
        execution_time = time.time() - start_time
        
        print(f"✅ Блок 05 выполнен за {execution_time:.2f} секунд!")
        print(f"📊 Результат восстановления:")
        print(f"   - Восстановлено quotes: {len(quotes_df)} строк")
        
        # Проверяем результат
        if not quotes_df.empty:
            print(f"   - Временной диапазон: {quotes_df['ts_ns'].min()} - {quotes_df['ts_ns'].max()}")
            print(f"   - Биржи в quotes: {quotes_df['exchange'].unique()}")
            
            # Статистика по биржам
            for exchange in quotes_df['exchange'].unique():
                ex_count = len(quotes_df[quotes_df['exchange'] == exchange])
                print(f"   - {exchange}: {ex_count} quotes")
        
        print("✅ Блок 05 успешно протестирован!")
